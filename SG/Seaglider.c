...
SEAGLIDER SETTINGS                                                
** SEAG.DIVENUM   Defines the current dive number of the Seaglider. Read in with each depth reading. Updated at beginning of dive
** SEAG.POWERON   Defines Logger Start Depth
** SEAG.POWEROFF  Defines Logger Stop Depth

WISPR SETTINGS
** WISP.GAIN      WISPR Gain Level [0-3]
** WISP.DETMAX    Maximum # of detections returned per MPC.DETINT Timer [-1 - MAX_DETECTIONS]
** WISP.DUTYCYCL  Integer number of duty cycle percentage. [0 - 100]
** WISP.DETNUM    # of dtx per call to initiate realtime upload of files. //Not Implemented on Seaglider
...
WriteFile (ulong TotalSeconds)
{
  ...
  sprintf (buf,
	   "===%s Program %3.2f===\nDiveNum:%d\nStartups:%d of %d\nWriteTime:%s\nTotalTime:%lu\nMaxDepth:%6.2fM\nDetInt:%3d\0",
	   MPC.PROGNAME, PROG_VERSION, SEAG.DIVENUM, MPC.STARTUPS,
	   MPC.STARTMAX, Time (NULL), TotalSeconds, max_dep, MPC.DETINT);
  flogf ("\n%s", buf);
...
UploadFile (TUPort * Port)
{
  ...
  for (BlkNum = 0; BlkNum <= NumBlks; BlkNum++)
	...
	BlkLength = LastBlkLength;
      ...
      if (buf == NULL)
	{
	  flogf ("CAN'T ALLOCATE \"buffer\". Reason:( %s )",
		 strerror (errno));
      ...
      TUTxFlush (Port);
      TURxFlush (Port);
      TUTxPutBlock (Port, buf, BlkLength, 5000);	//Dump the contents of the buffer through the serial line
...
** SG Depth Errors:
   ...
   2: In Status 6, waiting to upload, starts descending without receiveing upload
\******************************************************************************/
int
SG_Depth_Check ()
{
  ...
  if (stringvalue != 0)
    {
      if (stringvalue == 2)
	{			//Since SG2.3 Version: Uploadfile() was placed here. So efficient it gets called twice. Need
	  //to check data on Seagcom to see if it looks correct or if it looks overwritten. 
	  //**********
	  //********
  ...
  switch (SG_STATUS)
      ...
      else
	{
      ...
      //Wait for Glider to Begin Ascending at bottom of dive cycle request WISPR Free Space.  
    case 3:
    ...
    case 4:
      ...
      //If max_depth is updated here... Means we are still descending, double check and if needed boot status=3
      if (AverageFIFO () > 0)
	{
	  max_dep = Depth;
	  if (AverageFIFO () > 0)
...
SG_Data ()
{
  ...
  if (Surfaced)
      ...
      else if ((parsestring = strstr (returnline, "PAM")) != NULL)
	{
	  ...
	  SaveParams (newstring);	//SAVE THESE PARAMETERS TO SYSTEM.CFG FILE. CRC CHECK?
	  ParseStartupParams ();
//            SG_STATUS=1;
	  return 3;
	}
  ...
  split_divenum = strtok (NULL, "*");	//Tokenize at the comma to get the dive number from string
  ...
  if (depth_check == 0.00)	//If depth is exactly 0.00 
    return -4;
  else if (depth_check == Prev_Depth)	//If we get the exact same depth as least time, return bad depth
    return -5;
...
ParseTime (char *timein)
{
  ...
  sscanf (timein, "%d/%d/%d,%d:%d:%d", &month, &day, &year, &hour, &min,
	  &sec);
