...
** 6/1/2015
** Alex Turpin
** Plans to update DBG to include all console use. (.APP won't also be looking
for
**    CONS input)
...
SETTINGS                                                 Parse Character, Bytes
Taken, min and max values
...
void SetupHardware(void) {
  ...
  flogf("\n----------------------------------------------------------------\n");
  flogf("\nProgram: %s,  Version: %3.1f,  Build: %s %s", __FILE__, SG_Version,
        __DATE__, __TIME__);
  ...
  flogf("\nProject ID: %s   Platform ID: %s   Location: %s   \nBoot ups: %d",
        SG.PROJID, SG.PLTFRMID, SG.LOCATION, SG.STARTUPS);
...
void SGSystemStart() {
  ...
  // Turn on A to D Logging for Power Consumption and return system time
  // interval in minutes
...
/************************************************************************************************************************\
** void System_Timer();
** This function deals with different triggers to our alarm clock function and
sets them active.
** Variables:
...
void Sleep(void) {
  ...
  while (PinRead(IRQ5) && PinRead(IRQ4RXD) && PinRead(IRQ2) &&
...
** 3)Power data
\******************************************************************************/
int WriteFile(float Ah, float voltage, float max_dep, char *fname,
              ulong TotalSeconds, ulong uploadseconds) {
  float energy = 0.0;
  ...
  fprintf(uploadfile, "\nMax Depth: %.2f Meters\nCurrent: %.3fA\nVoltage: "
                      "%.2fV\nEnergy: %.1fkJ\nTime: %lu Seconds\nUpload Time: "
                      "%lu Seconds\nWISPR FS:%.2f%%\nDetections: %d\n",
          max_dep, Ah, voltage, energy, (ulong)TotalSeconds,
          (ulong)uploadseconds, WISPRFreeSpace, TotalDetections);
  flogf("\nMax Depth: %.2f Meters\nCurrent: %.3fA\nVoltage: %.2fV\nEnergy: "
        "%.1fkJ\nTotal Time: %lu Seconds\nUpload Time: %lu Seconds\nWISPR "
        "FS:%.2f%%\nDetections: %d\n",
        max_dep, Ah, voltage, energy, (ulong)TotalSeconds, (ulong)uploadseconds,
        WISPRFreeSpace, TotalDetections);
...
int UploadFile(char *fname, TUPort *Port) {
  ...
  // cprintf("\nNumber of Blocks: %d, Last Blk Length: %d", NumBlks+1,
  // LastBlkLength);
...
void Open_File(long *fcounter) {
  ...
  fnew = GetNextRECFileName(true, true, &counter); // Find the next file number
                                                   // 0000xxxx.dat. Don't
  ...
  if (hunt) // only do this when requested (usually just at first mount)
  {
    ...
    if ((err = DIRFindFirst(path, &de)) != dsdEndOfDir) {
      do {
        if (err != 0)
          break;
        if (de.d_name[9] == 'L' && de.d_name[10] == 'O' &&
            de.d_name[11] == 'G') {
          for (i = val = 0; i < 8; i++)
  ...
  if (hunt) // only do this when requested (usually just at first mount)
  {
    ...
    if ((err = DIRFindFirst(path, &de)) != dsdEndOfDir) {
      do {
        if (err != 0)
          break;
        if (de.d_name[9] == 'D' && de.d_name[10] == 'A' &&
            de.d_name[11] == 'T') {
          for (i = val = 0; i < 8; i++)
            if (de.d_name[i] >= '0' && de.d_name[i] <= '9')
              val = (val * 10) + de.d_name[i] - '0';
