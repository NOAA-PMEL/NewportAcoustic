/******************************************************************************\
**	CSACComTest.c				Persistor and PicoDOS starter C file  
**
...
TUSimpleTerm (void)
{
  ...
  cdrain ();
  coflush ();
//      uprintf("First term is for GPS.  To move on to Iridium term, press interrupt button.\n");
  cdrain ();
  coflush ();
//
...
//*****************************************************************************************
// GetSyncRTCbyCSAC()
// Get CSAC time in sec since 1970, Jan 1.  If set == true, lock RTC time to the CSAC time.  
// If set==false, just returns the CSAC sec.
// Char array time_chr contains time.
// H. Matsumoto 03-27-2013
//*****************************************************************************************
bool
GetSyncRTCbyCSAC (void)
//Get CSAC time, set the real time clock if set is true
{
  ...
  PIOSet (CF2CSAC);
  PIOClear (PCCSAC);
  PIORead (48);			//This statement is necessary since RX pin is connected to pin 48.
...
//Get GPS time, set the real time clock if set is true
{
  ...
  *RTCms = TICKS_TO_MS (ticks);
  ...
  cprintf ("RTC  %d %d:%d:%d:%d:%.3u\n", rtctime->tm_year, rtctime->tm_yday + 1, rtctime->tm_hour, rtctime->tm_min, rtctime->tm_sec, *RTCms);	//TICKS_TO_MS(ticks));
...
*************************************************************************************/
ulong
WhatTime (char *GMT_char, ushort * RTCms)
{
...
#if CFX == CF2
/******************************************************************************\
**	CustomRTCSyncStart	// Simple custom patch that syncs on PPSIN pin going high
\******************************************************************************/
static short TimeStartLevelDetect;
void CustomRTCSyncStart (void);
void
CustomRTCSyncStart (void)
{
...
*******************************************************************************/
void
RTCCSACSync (ulong secs, short detect)
{
...
#elif CFX == CF2
  {
    vfptr savedRTCSyncStart =
      BIOSPatchInsert (RTCSyncStart, CustomRTCSyncStart);
