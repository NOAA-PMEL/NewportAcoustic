/******************************************************************************\
**	cfxmain.c				Persistor and PicoDOS starter
C file
**
*****************************************************************************
**
...
#define STARTDEPTH_DESC "Start Depth\n"

#define DEPTHINTERVAL_NAME "DEPTHINT"
#define DEPTHINTERVAL_DEFAULT "25" // cm/s
#define DEPTHINTERVAL_DESC "Change in depth\n"
...
void CTD_Data() {
  ...
  } else if (WinchMode == 2) { // Going Down.
    depthchange = (float)(DEPTHINT + ((rand() >> 12) - 8));
    flogf("\ndepthChange: %f", depthchange);
    secs = RTCGetTime(NULL, NULL);
    timeChange = secs - prevTime;
...
\*************************************************************/
void AModem_Send(short reply, bool command) {
