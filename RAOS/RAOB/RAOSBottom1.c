...
**       G:    WISPR Gain
**       C:    DutyCycle ? Percentage or decimal? 0.5 vs 50% of ADCounter for each DETINT
**             >  A DETINT of 60 minutes would produce ADCounter==70;
**             >  The first half of the ADCounter (35 counts) the WISPR would be left off
**             >  The second half of that 60 minute Detection Interval WISPR would be on
**
...
**    
** -  Implement (near) Real-Time Detections by Calling to land after # of detections 
**       (parameter 'N') is surpassed by the number of returned detecions during one
**       Detection interval cycle (Parameter 'I').
**    >  This should also mean quicker updating of parameters to the Bottom system
**       including GPS time. RAOSBottom leaves WISPR off and AModem ON during RAOSTop
**       Iridium call, waiting for Top to call Bottom, timing out after certain wait period
**    
** 
...
**    >  Subtract power calculated by Power Monitor Plus defined error (POWERERROR) of 
**       some perecentage (1.05 = 5%)
**    >  Using 2 packs of 8 (parallel) by 10 (series) D Cell Alkalines we achieve 15v and roughly
**       288 AH. This pans out to be about 13MJ. (288AH * 12.5 (avg conservative voltage) *3600seconds per hour).)
**
**
...
//Main Program Loop. Only interrupted by Console Input or Vitals to set in Long Term Hibernation.
  while (SystemOn)
    {
      ...
      while (!BT_LP)
	{
	  ...
	  if (init_flag == false)
	    {
	      ...
	      //ParseStartupParams(); //This takes place only when new parameters have been received.
	  ...
	  //Get SystemTimeOn, Finish Logging Power, Update System of Average Voltage, Start Power Logging 
	  Time (&PwrOff);
	  TimeOn = PwrOff - PwrOn;
	  PwrOn = PwrOff;
	  ...
	  //Write Engineering File for this power cycle to be uploaded to the Top Acoustic Modem.
	  WriteFile (TimeOn);
	  ...
	  //count is just used for debugging process of waiting for AModem to respond. Might be too quick.
	  while (DataAlarm)
	    {
...
InitializeRAOS (ulong * PwrOn)
{
  ...
  uchar mirrorpins[] =
    { 1, 15, 16, 17, 18, 19, 21, 24, 25, 26, 28, 31, 32, 35, 36, 37, 42, 0 };
  ...
  *PwrOn = time;
  ...
  //Get Free Disk Space
  SystemFreeSpace = Free_Disk_Space ();	//Does finding the free space of a large CF card cause program to crash? or Hang?
...
/************************************************************************************************************************\
** void Incoming_Data_AModem()
** This function serves for serial interface when DataAlarm=true and dealing with the Acoustic Modem data transfer.
** We need this 
\************************************************************************************************************************/
void
Incoming_Data_AModem ()
...
WriteFile (ulong TotalSeconds)
{
  ...
  sprintf (buf, "\n===%s Program %3.2f===\nWrite Time:%s\nFile Number:%ld\n"
	   ...
	   MPC.PROGNAME, PROG_VERSION, Time (NULL), MPC.FILENUM,
	   filenumber, SystemFreeSpace, TotalSeconds,
	   MPC.STARTUPS, MPC.STARTMAX, MPC.DETINT, MPC.DATAXINT);
