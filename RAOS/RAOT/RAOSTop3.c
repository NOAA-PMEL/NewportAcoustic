...
**       G:    WISPR Gain
**       C:    DutyCycle giving in integer to be represented as percentage (0 -> 100)
...
**    >  This would gives us an idea of what is happening right now. Plus, we need to 
**       see what detection are being returned if those detections are invoking this call.
**
** 
	  ...
	  if (SystemOn)
	    {
	      ...
	      RTCDelayMicroSeconds (5000000L);	//Wait to make sure Bottom gets last DONE sent
	    }
...
InitializeRAOS (ulong * PwrOn)
{
  ...
  uchar mirrorpins[] =
    { 1, 15, 16, 17, 18, 19, 24, 25, 26, 28, 31, 32, 35, 36, 37, 42, 0 };
...
Incoming_Data (void)
{
  ...
  while (incoming)
      ...
      if (tgetq (AModemPort))
	{
	  ...
	  else if (amodemreturn == -4 && AModemPhase == 4)
	    {			//only comes here when serial data received. Go to AModemStream.
	      AModemSend ("RESEND");
	      StreamLoop (2);
	    }
...
Sleep_AModem (void)
{
  ...
  IEVInsertAsmFunct (IRQ5_ISR, level5InterruptAutovector);	//BT (PAM Port) Intterupt
...
WriteFile (ulong TotalSeconds)
{
  ...
  if (filehandle <= 0)
    {
      flogf ("\nERROR  |WriteFile() %s open error: errno: %d", uploadfile,
	     errno);
      if (errno != 0)
	return -1;
    }
  ...
  sprintf (WriteBuffer,
	   ...
	   SystemFreeSpace, TotalSeconds, MPC.STARTUPS, MPC.STARTMAX,
	   MPC.DETINT, MPC.DATAXINT);
#ifdef DEBUG
  flogf ("\n%s", WriteBuffer);
  cdrain ();
  coflush ();
