...
#define  COM4TX      25
#define	SYSCLK	10000		// choose: 160 to 32000 (kHz)
#define  LOWCLK   3200
...
int main()
   ...
   flogf("At COM4, type ^C to connect GPS and COM4. Antenna SW is GPS side.\n");
   flogf("At COM4, type ^D to connect Iridium and COM4. Antenna SW is IRID.\n");
   flogf("To exit, type ^E\n");
   ...
   */
   ...
   flogf("To switch from SB39 to GPS, type ^C\n");
   putflush();
...
//DBG(flogf("Elapsed time %ld us\n", RTCElapsedTime(&tmtset)));
   ...
   TURxFlush(COM4Port);
	...
	PITSet100usPeriod(PIT1kHz);
	TMGSetSpeed(LOWCLK);
		
	while(chin !=2){           //Wait until get CTRL b
      //while (PinRead(IRQ5)) 
   	while (PinTestIsItBus(IRQ5)) 
   		{
	...
	flogf("Detected %x\n", chin);
  	/*
  	tputc(COM4Port,'$');
  	chin=2;//default SB19plus mode
  	flogf("$ sent\n");
  	*/
  	
  	if(chin==2)flogf("Got CTRL b\n");
	while(chin!=5)
	{//= CTRL E to exit
      if(chin==2)
      {//CTRL B to communicate with SBE39
      
         flogf("CTL B entered\n");       
         chin=0x0A; //clear chin
DBG(RTCElapsedTimerSetup(&tmtset));
        	while(chin>7)
        	{//25 us  //printable ASCII
         	...
         	if (tgetq(COM4Port))
         	{// we've got data at COM4 port from the main
         		chin=tgetc(COM4Port);
...
if(test==0)DBG(flogf("Elapsed time %ld us\n", RTCElapsedTime(&tmtset)));
        test=1;                  
         }
      }    
         
      if(chin==3){// CTRL C GPS
         flogf("GPS mode\n");
         if(IRIDGPSPort==0)OpenIRIDGPSPort(true);
         PIOClear(1); RTCDelayMicroSeconds(1000000L);
      ...
      if(chin==4){// CTRL D Iridium 
         flogf("Iridium mode\n");
         if(IRIDGPSPort==0)OpenIRIDGPSPort(true);
         chin=0x0A;//clear chin
         PIOSet(1); RTCDelayMicroSeconds(1000000L);
        	while(chin>7){        	
         	if (tgetq(IRIDGPSPort))//receive data from IRIDGPS modem A3LA
         		{					  
               tputc(COM4Port, tgetc(IRIDGPSPort));
         		//cputc(chin);// grab it and show it
         		}

         	if (tgetq(COM4Port))//Receive data from the main 
         		{// we've got data COM4 port
         		chin=tgetc(COM4Port);
         		if (chin >7){//normal ASCII
            		   tputc(IRIDGPSPort,chin);
   ...
   if(SB39Port    !=0) OpenSB39Port(false);
   if(IRIDGPSPort !=0) OpenIRIDGPSPort(false); 
   if(COM4Port    !=0) OpenCOM4Port(false);
   PIOClear(COM4PWR);
  	PIOClear(ADCPWR);   
  	cdrain(); coflush();	
   free(SB39send);
...
void OpenIRIDGPSPort(bool on)
{
   ...
   if(on)flogf("Opening the high speed IRID/GPS port\n");
	if(on){
...
void OpenCOM4Port(bool on)
{
   ...
   if(on)flogf("Opening the high speed COM4 port\n");
	if(on){
...
//<Executed/>
   ...
   sprintf(SB39send, "$ATD %s, %s, %s, %s*", split_temp, split_pres, \
...
/******************************************************************************\
**	Irq5RxISR			Interrupt handler for IRQ5 (tied to CMOS RxD)
**	
