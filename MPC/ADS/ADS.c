...
Updates from ADS.c version 2.0
   ...
   -Received many errno=0 when fopen return NULL. No idea why. waiting to hear back from JG 12.2.2015

Updated from ADS.c version 1.5 
   -with more efficient storing and calculating of current and voltage.

Version 1.5:
Storing "shorts" in circular array, averaging once every 30 seconds to another array of floating point values,
after 5 minutes averaging those 20 different 8-byte floating point values and writing to file. 

Version 2.0:
Summing "shorts" for giving sampling interval which lasts #seconds=(2^#bits*PITRATE*PITINT)
Here we can average a large number of samples at a quick rate due to bit shift division. At the end of every
average, we write the three different "ushort" values (current, voltage, and time of sampling) to the power file.
Only when power monitor is called does the power file sum its "ushort" values and divided again ( non-bit shifted integer division)
before it is converted to a floating point value with "CFxADRawToVolts(...)" This makes for a very fast and efficient power logging 
process.

   A) Need to check which BITSHIFT size leads to overflow.
      1) BITSHIFT of 10 results in 25.6 secon buffers, 11: 51.2, 12: 102.4 etc. 16? Too big??
      
   B) No required Array allocation or "data2" for knowing power needs to be averaged.
      2) Just checking to see if "data" is true will spark averaging.

   C) ADSTIME will be saved as a "ushort" and can be divided by "10.0" to transfer into the real power write time.
...
//Total Number of samples per ADSTIME. Defined by BITSHIFT and sampling frequency
ushort   SAMPLES;
ushort 	BitShift;
...
long     Nsamps;	//Incremented upon each sample. Once Nsamps equals SAMPLES (2^BITSHIFT)
...
bool AD_Check(){if(data==true&&ADSOn){TickleSWSR(); AD_Log(); return true;} else return false;}
bool ADS_Status(){ return ADSOn;}
int Get_ADCounter(){ return ADCounter;}
void Reset_ADCounter(){ ADCounter=0;}
void ADSFileName(long counter){ sprintf(&ADAvgFileName[2], "%08ld.pwr", counter);}
/**********************************************************\
** Void SetUpADS()
...
void Setup_Acquisition(ushort BitShift)
{
   ...
   // Initialize AD Slot and Lock	
	...
	if(! CFxADLock(ad)){flogf("\nCouldn't lock and own A-D with QSPI\n");return;}//Lock in the ADDataBuf 
...
** 2)Checks both booleans and writes correct side of AD Buffer to file (doesn't acutally
** save file, rather averages each side of buffer and stores one averaged value for set buffer period)
...
/**********************************************************\
** AD Write
** Open file of Current averages, go to end of file and grab last averaged reading.
...
CLK(  stop_clock = clock();   print_clock_cycle_count(start_clock, stop_clock, "AD_Write: open");      )
   
CLK(   start_clock = clock();)
  
   write(ADSFileHandle, AveragedEnergy, 3*sizeof(ushort));
   RTCDelayMicroSeconds(25000);
CLK(   stop_clock = clock();   print_clock_cycle_count(start_clock, stop_clock, "AD_Write: write");      )
...
** With a FWT for the ADS of 32seconds and a WRTINT of ~60 minutes (really 64 minutes)
\******************************************************************************/
float Power_Monitor(ulong totaltime, int filehandle, ulong* LoggingTime){
   ...
   if(filelength<6) return 0.0;
   
   if(ADSFileHandle>0){
   	lseek(ADSFileHandle, 0, SEEK_SET);
	   filelength=filelength/6; //6 is the number of bytes for the values of current, voltage, time.
...
** 2) While ADRawHead is Less than the total buffer size, take sample and store in buffer
...
IEV_C_FUNCT(ADTimingRuptHandler)	// implied (IEVStack *ievstack:__a0) parameter
{
   ...
   ADSample= (short*) CFxADQueueToArray(ad, (void *) QRR, 2); //QSPI driver function to sample '2' channels from 'ad' and save into ushort array 'ADSample'
