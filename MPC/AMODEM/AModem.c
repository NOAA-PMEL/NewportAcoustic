...
short AModem_Data(){
   ...
   //if Done Close tuport   
      ...
      if(AModemPhase==2){
         ...
         //AModemReset(); //AModemPort Reset to help save power. Instead of wasting 
         //AModemAlarm=false; //Operating in Mode 1 (Near Real-Time operation) we can to Shutoff AModemPort after receiving new Parameters from IRID
         }
...
DBG(         flogf("\n\t|Post AModemUPload");)
   ...
   //if RESEND, resend last block   
      ...
      AModemSend("DONE");
      if(NewSystemParams) ParseStartupParams();
      Delay_AD_Log(8);
      AModemPhase=6;
      return 6;
      }
   ...
   else if(strstr(ComString, "-1")!=NULL){ return -1; TURxFlush(AModemPort); TUTxFlush(AModemPort);} 
   else if(strstr(ComString, "-2")!=NULL) return -2;
   else if(strstr(ComString, "-3")!=NULL) return -3;   

   DBG(flogf("\nData Fail: %s", ComString);)
   
   if(ResendCount>=MAX_RESENDS){AModemPhase=6; return -6;}
...
while(fname!=NULL){
   ...
   stat(fname, &fileinfo);   
DBG(   flogf("\n\t|File Size: %ld\n\t|Maxuploadremaining: %ld",fileinfo.st_size, maxuploadremainder);) //Log the IridFile length
   ...
   NumBlks++;
DBG( flogf("\n\t|Number of Blocks: %d \n\t|Last Block Length: %hu", NumBlks, LastBlkLength);)
...
DBG(   	else flogf("\n\t|AModemUpload() %s Opened", fname);)
   	   ...
   	   if(BlkNum==NumBlks) BlockLength=LastBlkLength;
...
DBG(     flogf("\n\t|crc: %#4x\n\t|Blklen: %d",crc_calc, blklen); putflush();CIOdrain();RTCDelayMicroSeconds(20000L);)
...
//DBG(  flogf("\n%s", AModemBuffer); )
      ...
      BlkNum++;
 	   ...
 	   if(amodemreturn==1){	//If "ACK"
         ...
         flogf("\n%s|AModem_Upload(): Bad return from AModem_Data(): %d", Time(NULL), amodemreturn);
         
      else
         flogf("\n%s|AModem_Upload(): AModemPort unresponsive: %d", Time(NULL), amodemreturn);
...
int AModemStream(int filehandle){
 ...
 //  flogf("\n\t|AModemStream()"); cdrain(); coflush();
	...
	while(input!='@'&&input!='$'){ 
	   ...
	   count++;
	   if(count>AMDM.BLKSIZE){command=true; string="ERR"; break;}
	   }
	if(input=='$'){
		command=true;
	   string= AModemCom();
...
DBG(flogf("\n\t|cmdLength: %d, qsize: %ld", cmdLength, qsize);)
   ...
   if(cmdLength>AMDM.BLKSIZE){
      ...
      DBG(flogf("\n\t|False cmdLength of: %hu, changed to bytes in queue: %ld", cmdLength, qsize);)
 ...
 //  flogf("\n\t|Grabbing %hu Byte", cmdLength);
   ...
   bytesreceived = TURxGetBlock(AModemPort, AModemBuffer+5, (long)cmdLength-5, 20*cmdLength); //Testing 7.15.2015, commented section below works well. Trying to streamline
   
DBG( flogf("\n\t|bytesreceived: %ld", bytesreceived);)   
//DBG(flogf("\n\n%s", AModemBuffer);)
   crc_comp = CalcCrc(AModemBuffer, cmdLength);
   DBG(flogf("\nCalculated Crc: %#4x, Received Crc: %#4x",crc_comp, crc_chk); putflush();CIOdrain();RTCDelayMicroSeconds(20000L);)
	
   if(crc_comp==crc_chk){
      flogf("\n%s|Successful Amodem Download Block %X of %hu", Time(NULL), AModemBuffer[3], AModemBuffer[4]);
...
DBG(  flogf("\n\t|Bytes Written: %d", byteswritten);)
      ...
      flogf("\nERROR|Bad CRC Check. Resend Last Block"); cdrain(); coflush();
      AModemSend("RESEND");
      Delay_AD_Log(1);
      TUTxFlush(AModemPort);
      TURxFlush(AModemPort);
      if(ResendRequest>3) return -3;
     	else return -2;
      }
   ...
   short count=0;
	...
	if(chars=="@@@"){
   	while(input!='@'&&input!='$'){ 
   	   ...
   	   count++;
   	   if(count>AMDM.BLKSIZE) return "ERR";
   	   }
   	if(input=='$'){
   	   return AModemCom();
   	   }
...
const char* AModemCom(){
   ...
   r[0]='$';
	...
	if(r[1]==-1){DBG( flogf("\n%s|AModem_Data():The first input character is negative one",Time(NULL)); RTCDelayMicroSeconds(20000L); ) return "-2"; }
	for(i=2;i<64;i++){	//Up to 62 characters	
	   in=TURxGetByteWithTimeout(AModemPort,100);
      if(in == '*'){ r[i] = in; break;} //if we see an * we call that the last of this WISPR Input
		else if(in == -1){ return "-3";}
	   else r[i]=in;
   }
...
void AModemUploadParameters(){
   ...
   DBG(flogf("\n\t|Bytes Read from %s: %ld", RAOSParamFile, bytesread);cdrain(); coflush();)
   if(close(file)<0)
...
void AModem_SetPower(bool high){
	...
	TUTxPrintf(AModemPort, "~"); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "\%"); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "L"); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "Q"); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "U"); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "W"); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "M"); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "."); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "C"); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "O"); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "M"); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "D"); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "3"); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "2"); RTCDelayMicroSeconds(waittime);
	TUTxPrintf(AModemPort, "0"); RTCDelayMicroSeconds(waittime);
	if(high){
		TUTxPrintf(AModemPort, "1"); RTCDelayMicroSeconds(waittime);
		}
	else{
		TUTxPrintf(AModemPort, "0"); RTCDelayMicroSeconds(waittime);
		}
