...
void SetupHardware(void)
   ...
   TickleSWSR();
	...
	flogf("\n----------------------------------------------------------------");
	flogf("\nProgram: %s,  Version: %3.2f,  Build: %s %s", 
		__FILE__, PROG_VERSION, __DATE__, __TIME__);
...
void Make_Directory(char *path)
{
   ...
   flogf("\n%s|MakeDirectory() %s",Time(NULL),DOSCommand);putflush();CIOdrain();
   execstr(DOSCommand);
...
void DOS_Com(char *command, long filenum, char* ext, char* extt)
{
   ...
   flogf("\n%s|COMDos() %s",Time(NULL),Com);putflush();CIOdrain();
   execstr(Com);	
   ...
   if(remainder==0){
      ...
      //CTD Measurements
			...
			if(depth<=NIGK.TDEPTH){	
				...
				//Still Less than target depth? Set in recovery mode. Set calling interval to 30 minutes. 
				if(depth<NIGK.TDEPTH){
					flogf("\nERROR|System_Timer(CTD): DEPTH LESS THAN TARGET DEPTH!");	//This will be interest to happen. Most likely detachment.
					//Set NIGK to recovery?
					NIGK.RECOVERY=1;
					VEEStoreShort(NIGKRECOVERY_NAME, NIGK.RECOVERY);
					MPC.DATAXINT=30;
					VEEStoreShort(DATAXINTERVAL_NAME, MPC.DATAXINT);//Call every 30 minutes at surface.
					return 2;
					}
				}
		#endif
...
** void AppendFile
\***************************************************************************************/
bool Append_Files(int Dest, const char* SourceFileName, bool erase, long maxBytes){
...
DBG(else
   ...
   flogf("\n\t|Append_Files %s Size: %ld Bytes", SourceFileName, fileinfo.st_size);
   ...
   for(i=0;i<=NumBlks;i++){
      ...
      if(i==NumBlks) BlockLength = LastBlkLength;
...
DBG(  flogf("\n\t|AppendFiles: bytes read: %d", byteswritten);)
      if(i==NumBlks){ strcat(WriteBuffer, "\n"); BlockLength++;}
...
DBG(     flogf("\n\t|Append_Files() %s Closed", SourceFileName);)
         sprintf(extension, "%c%c%c", SourceFileName[11], SourceFileName[12], SourceFileName[13]);
         DOS_Com("del", MPC.FILENUM, extension, NULL);
         }
      else
         flogf("\nERROR  |AppendFiles() %s close errno: %d",SourceFileName, errno);
      }
   else{
      if(close(Source)!=0)  flogf("\nERROR  |AppendFiles() %s close errno: %d",SourceFileName, errno);
...
**
\******************************************************************************/
char *GetFileName(bool Lowest, bool incIndex, long *fcounter, const char* FileType){
   ...
   //Do this with *log extension next
	if ((err = DIRFindFirst(path, &de)) != dsdEndOfDir)
		...
		do	{
			...
			if (de.d_name[9] == FileType[0] && de.d_name[10] == FileType[1] && de.d_name[11] == FileType[2])
				{
				filecounter++;
				for (i = val = 0; i < 8; i++)
...
bool SaveParams(const char* Command){
   ...
   memset(params, 0, 64);
   flogf("\n\t|SaveParams(%s)", Command); cdrain(); coflush();
...
** 	-Param1: if true load default.cfg settings file. if false, load system.cfg
\***********************************************************************************************/
void ParseStartupParams(bool DefaultSettings){
   ...
   flogf("\n\t|ParseStartupParams():");cdrain();coflush();
			...
			case 'M':case 'm':
				if(value==1||value==0)
					if(value!=IRID.CALLMODE){
						flogf("\n\t|Changed Iridium Call Mode: %s", value ? "Time of Day" : "Data Interval");
						IRID.CALLMODE=value;
...
void print_clock_cycle_count(clock_t start, clock_t stop, char* label){
   ...
   flogf("\n%f seconds for %s", ((double)(stop-start))/(double)CLOCKS_PER_SEC, label);
