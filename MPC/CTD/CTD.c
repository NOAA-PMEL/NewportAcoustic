...
int Sea_Ice_Algorithm();   //Returns reason: 1=Stop, should be at surface and ready to send.
...
//	strftime(buffer, 15, "%m%d%Y%H%M%S", info);
	sprintf(buffer, "%02d%02d%04d%02d%02d%02d", info->tm_mon, info->tm_mday, info->tm_year+1900, info->tm_hour, info->tm_min, info->tm_sec);
	printf("\nCTD_DateTime(): %s\n");
...
bool CTD_GetPrompt(){
   ...
   LastByteInQ=TURxPeekByte(CTDPort, (tgetq(CTDPort)-1));     
 	while(((char)LastByteInQ != '>')&&count<2){  //until a > is read in command line for CTDPort or 7 seconds pass                  
     	TUTxPrintf(CTDPort, "\n");                           
   ...
   if(count==2){
   	...
   	if(strstr(stringin, "S>")!=NULL){cprintf("\nPrompt from CTDBlock"); TURxFlush(CTDPort); return true;}
   	return false;
   }	
...
** The sea ice algorthim starts a careful ascent when the buoy reaches the threshold depth
...
**    3. 
** if (t_change of <.5 && Sal_change <.5). stop. change to continuous sampling, ascend
...
int Sea_Ice_Algorithm(){
  ...
  /* 
   ...
   sal_change=(sal_change+ (next_sal-current_sal));    //Calculate salinity change
   ...
   //Simulation Mode
   if(sim==true){
      if(SimDepth<=Depth_Threshold&&BuoyMode==1){  cprintf("\nSIA: Depth Threshold Reached");
           
         if(temp_change<=0.5&&(abs(sal_change)<=0.5)){ 
            cprintf("\nSIA: Small variation between TEMP and sal ");
            CommandWinch("\#S,01,00", true);   RTCDelayMicroSeconds(2000000); //try get command to get reult back? so no delay needed
            if(temp<=Temp_Threshold){ CommandWinch("\#F,01,00", true);
               RTCDelayMicroSeconds(2000000);       cprintf("\nSIA: Minimum Temperature Recognized ");
               ice=4; }
            else if(temp>Temp_Threshold&&BuoyMode==0){              cprintf("\nSIA: Careful Ascent ");
               ice=3;              CommandWinch("\#R,01,02", true);         RTCDelayMicroSeconds(2000000);     }
         }
   ...
   //Real Dive Sea Ice Algorithm Mode      
   else if(pres<=Depth_Threshold&&BuoyMode==1){          //Depth threshold reached
      if(temp_change<0.5&&sal_change<0.5){               //if change in ctd readings is small 
         CommandWinch("\#S,01,00", true);                     //Stop winch
         if(temp<=Temp_Threshold){                       //if temp is below threshold 
            CommandWinch("\#F,01,00", true);                  //descend
            cprintf("SIA: Minimum Temperature Recognized ");
            ice=4;        }
            else if(temp>Temp_Threshold&&BuoyMode==0){   //if temp is above threshold
            CommandWinch("\#R,01,02", true);                  //put in careful ascent mode
            cprintf("SIA: Careful Ascent ");
            ice=3;   }}      
      else if(pres<=Min_Depth){                          //if change in ctd readings is large, and buoy reaches minimum depth
      ...
      if(current_temp>Temp_Threshold){                   //temperatures reads ok
         ...
         ice=3; }                                        //no ice         
      else if(abs(sal_change)<0.4){                           //change in salinity is too small
         CommandWinch("\#S,01,00", true);                     //stop 
         ice=2;                                          //sal change bad
         CommandWinch("\#F,01,00", true);  }                  //descend
      else if(pres<=Min_Depth){                          //buoy reached minimum depth
         CommandWinch("\#S,01,00", true);                     //Should it ascend more if still no signal? depends on our iridium float attached to the buoy
        // Surfaced=true;                                  //surfaced =true go to iridium
         ice=1;}
  }
  */
   return ice;
...
bool CTD_Data(){  
	...
	memset(stringin, 0, 128*sizeof(char));
  	...
  	if(strchr(stringin, '#')!=NULL){ log=true;}
	else if(strchr(stringin, '>')!=NULL) prompt=true;
	else if(strchr(stringin, '<')!=NULL) prompt=true;
	
  	if(prompt){
 		flogf("\nERROR|CTD_Data(): Received >s rather than #, sending into sleep mode."); 	
		CTD_SyncMode();
  		Delay_AD_Log(3);
		TUTxPrintf(CTDPort, "TS\n");
		TUTxWaitCompletion(CTDPort);
   ...
   info.tm_mon=-1;
   mon = strtok(NULL, " ");
	if(strstr(mon, "Jan")!=NULL)info.tm_mon=0;
	else if(strstr(mon, "Feb")!=NULL)info.tm_mon=1;
	else if(strstr(mon, "Mar")!=NULL)info.tm_mon=2;
	else if(strstr(mon, "Apr")!=NULL)info.tm_mon=3;
	else if(strstr(mon, "May")!=NULL)info.tm_mon=4;
	else if(strstr(mon, "Jun")!=NULL)info.tm_mon=5;
	else if(strstr(mon, "Jul")!=NULL)info.tm_mon=6;
	else if(strstr(mon, "Aug")!=NULL)info.tm_mon=7;
	else if(strstr(mon, "Sep")!=NULL)info.tm_mon=8;
	else if(strstr(mon, "Oct")!=NULL)info.tm_mon=9;
	else if(strstr(mon, "Nov")!=NULL)info.tm_mon=10;
	else if(strstr(mon, "Dec")!=NULL)info.tm_mon=11;
	...
	month= info.tm_mon+2;
	sprintf(split_date, ", %d.%d.%d %d:%d:%d", info.tm_mday, month,info.tm_year-100,info.tm_hour,info.tm_min,info.tm_sec);
   ...
   //Log WriteString
  	if(log){
	  	...
	  	LARA.DEPTH = atof(strtok(split_pres, ","));
	  	strcat(writestring, ","); //added 9.28 after deploy 2, data 1. 02UTC of 9.29
...
float CTD_AverageDepth(int i, float* velocity){
	...
	if(tgetq(CTDPort)) TURxFlush(CTDPort);
   //This for loop is to understand the profiling buoy's starting position prior to ascending the water column.
   Delay_AD_Log(1);
   //TUTxPrintf(CTDPort, "\n");
   CTD_Sample();
   ...
   for(j=0;j<i;NULL){
   	...
   	if(AD_Check()){
   		if(j==0) 
   			CTD_Sample();
   		j++;
   		if(j==i){ *velocity = 0.0; return 0.0;}
			}
		if(tgetq(CTDPort)){
         if(CTD_Data()){
         if(firstreading) starttime=RTCGetTime(NULL, NULL);
	         depth[j]=LARA.DEPTH;
	         j++;
	         }
         if(j==i){stoptime=RTCGetTime(NULL, NULL); break;}
        	CTD_Sample();        	
         }
		}
   ...
   if(abs(depth[0]-depth[i-1])<1.0){
   	flogf("\n\t|Profiling Buoy Stationary at %5.2f at %5.2fm/sample", depth[0], vel);
   	returnValue= depth[0];
   	}
  	else{
  		flogf("\n\t|Profiling Buoy Moving currently at %5.2f at %5.2fm/sample", depth[0], vel);
  		returnValue= 0.0;
  		}
