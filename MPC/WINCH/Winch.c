      ...
      //if(!symbol)
	   ...
	   WINCH.DESCENTRCV++;
		...
		if(symbol){
      	...
      	if(LARA.BUOYMODE==1){//Winch Stops buoy from Ascending... Lack of Tension? Are we close to TDepth? Try Calling?
				depth=LARA.DEPTH-NIGK.TDEPTH;					
				flogf("\n\t|AModem_Data() Winch Stopped Buoy");
				if(depth>5.0&&AscentStopTries<2){	//if buoy still too deep. try ascending again
					Winch_Ascend();
					AscentStopTries++;
					}
				else if(depth<2.0||AscentStopTries>=1){	//if buoy less than 5 meters to target depth OR tried ascending at least twice before.
...
void Winch_Monitor(int filehandle){
	...
	sprintf(WriteBuffer, "NIGK %s\nProfile:%d\nDelay:%d\nTDepth:%d\nRiseRate:%d\nFallRate:%d\nASCENTCALLS:%d, RCV:%d\nDESCENTCALLS:%d, RCV%d\nSTOPCALLS:%d, RCV%d\n\0", \
	NIGK.RECOVERY ? "RECOVERY" : "ON", NIGK.PROFILES, NIGK.DELAY, NIGK.TDEPTH, NIGK.RRATE, NIGK.FRATE, WINCH.ASCENTCALLS, WINCH.ASCENTRCV,  \
	WINCH.DESCENTCALLS, WINCH.DESCENTRCV, WINCH.STOPCALLS, WINCH.STOPRCV);

	
	//Maybe include something about calculated velocities and cable length of winch after #of seconds.
	flogf("\n\t|Winch_Monitor():\n%s", WriteBuffer);
