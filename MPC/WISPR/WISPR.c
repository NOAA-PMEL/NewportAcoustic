...
short WISPR_Data(){ 
   ...
   //WisprString =
   GetWISPRInput(&returndouble); //changed to 1.25 second wait time to receive the '$'
  ...
  */    
      ...
      if(returndouble<MIN_FREESPACE){
         ...
         if(WISP.NUM<WISPRNUMBER){
            ...
            //Only location when to permanently increment wispr number: When free space runs out. or from start script.
...
DBG(     else
      ...
      if((int)returndouble<dtxrqst)dtxrqst = (int)returndouble;
...
DBG(     else
      ...
      DBG(flogf("\n%s", WisprString);)
         ...
         if(dtxrqst>0){
            for(i=0;i<dtxrqst;i++){
               ...
               TURxFlush(PAMPort);
   #ifdef ACOUSTICMODEM             //This if defined statement should be based on whether an AMODEM is implemented but rather a real-time detection turn around.
            if(WISP.DETNUM>0)
      ...
      flogf(": Found Exit");
      RTCDelayMicroSeconds(2000000L);  //Gives a little bit of time to WISPR to umount /mnt
...
void WISPRDet(int dtx){
   ...
   if(dtx<0) return;                //if negative number then don't call detections
...
5. will return number of detections if we get DXN. for which we will run a for loop to get each detection in this same function
...
char* GetWISPRInput(float* numchars){
   ...
   r[0]='$';
	...
	if(r[1]==-1){DBG( flogf("\nThe first input character is negative one"); RTCDelayMicroSeconds(20000L); ) return NULL; }
	for(i=2;i<64;i++){	//Up to 59 characters	
	   in=TURxGetByteWithTimeout(PAMPort,100);
      if(in == '*'){ r[i] = in; //TURxFlush(PAMPort); 
      break;} //if we see an * we call that the last of this WISPR Input
		else if(in == -1){TURxFlush(PAMPort); return NULL;}
	   else r[i]=in;
      }
...
void GetWISPRSettings(){
   ...
   DBG( uprintf("DUTYCYCLE=%d (%s)\n", WISP.DUTYCYCL, p ? "vee" : "def"); cdrain(); )
...
//   buf = (char*) calloc(256, sizeof(char));
   ...
   if(WISP.DUTYCYCL>0){    
      sprintf(WriteBuffer, "\n---WISPR%d---\nGPS Sends: %d\nGain:%d\nFree Space:%4.2f%%\nDuty Cycle:%d%%\nMax Detections:%02d\nTotal Detections:%d\n\0", 
         WISP.NUM, WISPRGPSSends, WISP.GAIN, WISPRFreeSpace, WISP.DUTYCYCL, WISP.DETMAX, TotalDetections);
...
** Read "wisprfs.bat" file. return free space of current WISPR? ||  return free space of last wispr
\***********************************************************************************************/
float GetWISPRFreeSpace(){   
...
DBG(else
   ...
   if(close(filehandle)<0)
      flogf("\nERROR  |create_dtx_file(): %s close errno: %d", SourceFileName, errno);
DBG(else
...
void GatherWISPRFreeSpace(){
   ...
   for(i=1;i<=WISPRNUMBER;i++){
      ...
      byteswritten = write(wisprfilehandle, wisprbuff, strlen(wisprbuff)*sizeof(char));
...
DBG(           flogf("\n\t|GWFS: DFP2");)
         ...
         if(tgetq(PAMPort)){
            wret=WISPR_Data();
            if(wret==1){ RTCDelayMicroSeconds(150000);
               wret = WISPR_Data();
               if(wret==5){ 
                  gain=true;
                  RTCDelayMicroSeconds(150000);
DBG(              flogf("\n\t|GWFS: DFP1");)
      ...
      TURxFlush(PAMPort);
      TUTxFlush(PAMPort);
      if(!dfp) WISPRFreeSpace=00.00;


      dfp=false; gain=false;
      count=0;
      wnum++;
      if(wnum>WISPRNUMBER)break;
...
DBG(else
   ...
   //flogf("\n\t|%s", WisprString);   
   if(WISP.NUM>WISPRNUMBER) WISP.NUM=WISPRNUMBER;
   sprintf(wispnum, "W%d:", WISP.NUM);   

   p = strstr(WisprString, wispnum);
   if(p==NULL)flogf("\nERROR  |No String search found for: %s", wispnum);
   length= p-WisprString;
   if(length>30){flogf("\nERROR |Returning due to bad filde position for %s", wisprfile);close(wisprfilehandle); return;}
...
bool WISPRExpectedReturn(short expected, bool reboot){
	...
	if(WISPR_Data()!=expected){
		OpenTUPort_WISPR(false); Delay_AD_Log(2);
		OpenTUPort_WISPR(true); Delay_AD_Log(1);
		switch(expected){
			case 1: WISPRGPS(); break;
			case 2: WISPRDFP(); break;
			case 3: WISPRDet(WISP.DETMAX); break;
			case 6: WISPRExit(); break;
			}	
		if(WISPR_Data()!=expected)
			if(reboot){
				WISPRSafeShutdown();
