...
//This structure is primarily for system diagnostics logging. Keeping track of phase, alarms, system statuses
typedef struct{

	short PHASE; 		//1=AUH, 2=Ascent Profile, 3=Surface Communitcation, 4= Descent Profile
	bool ON;				//While "ON", continue running program
	bool DATA;			//Data is triggered true when the timing interval goes off.
	bool SURFACED;		//Set to true when Antenna is estimated to be protruding from water
	short BUOYMODE;	//0=stopped 1=ascend 2=descend 3=careful ascent	
	float DEPTH;		//Most recent depth measurement from CTD
	float MOORDEPTH;	//Depth confirmation at beginning of LARA.PHASE==1. Averaged Samples
	short TDEPTH;		// 
	float TOPDEPTH;
	float AVGVEL;
	float PAYOUT;
...
//PowerLogging
#ifdef POWERLOGGING
   //LARA has 4 sets of Lithium-Primary Batteries. Rated at 14.4V (Initial at 14.74V) and 117Amp-Hour
