...
**	
*****************************************************************************
**	Before deploying, set CF2 time and SM2 time, format SD cards, replace SM2 and CF2 clock batteries
** erase activity.log, set status=0, set startups=0,
**	verify boottime is correct, verify pwrondepth and pwroffdepth are correct,
** make sure you have the correct mA multiplier set in the mAh calculation in status 5
** for the SM2 settings that you are using(compression, kHz, etc.) as power consumption
...
//#define LPMODE    FullStop          //NOT USING SLEEP MODE CURRENTLY// Low power mode
...
static char uploadfile[]="c:00000000.dat"; //12.9.2015 Can this be a static char?
...
void main(){
   ...
   ulong PwrOn, PwrOff;
	...
	//Allocation of Space for the Global buffer. Mostly used to write to the uploadfile.
	WriteBuffer= (char*)calloc(256, sizeof(char));
   
   //Platform Specific Initialization Function. PwrOn is the start time of PowerLogging
   ...
   //Main Loop. Always running unless interrupted by User input of '1' followed by 'y'. Or, if We have reached a full shutdown: below absolute minimum voltage.
  	while(LARA.ON){
...
void InitializeLARA(ulong *PwrOn){
   ...
   //First Safety Catch. If woken. Reboot
	...
	// Identify IDs
	flogf("\nProgram: %s  Project ID: %s   Platform ID: %s  Boot ups: %d", MPC.PROGNAME, MPC.PROJID, MPC.PLTFRMID, MPC.STARTUPS);
   ...
   *PwrOn=time;
   ...
   //Get Free Disk Space
   Free_Disk_Space(); //Does finding the free space of a large CF card cause program to crash? or Hang?
	...
	if(MPC.STARTUPS>0){
		MPC.FILENUM--;
	   sprintf(&uploadfile[2], "%08ld.dat", MPC.FILENUM);
		prevTime=WriteFile(0);	//By Sending 0 we understand this is the file being salvaged from a reboot when analyzing over RUDICS, also this zero is passed to Power_Monitor to properly calculate previous startups power consumption.
   ...
   //Else lower than user set minimum.
   	...
   	WISP.DUTYCYCL=0; //Booting up with default.cfg will never have a set value for turning on the WISPR. WISPR can only be turned on again if 
   ...
   //Startups>0 Go to default callmode @ callhour.   
   	...
   	GlobalRestart=true;
   	LARA.PHASE=0;	//Phase 0 when finding the winch status and ctd position as to place the Buoy in the proper phase
   	...
   	if(prevMode==0) hour = MPC.DATAXINT;
   	else hour =IRID.CALLHOUR;
   	...
   	if(depth>NIGK.TDEPTH){
   		...
   		if(velocity<-.05){
   			...
   			flogf("\n\t|At depth and no velocity. Phase 1 && BuoyMode 0");	//Stationary at depth
   			LARA.PHASE=1;
   			LARA.BUOYMODE=0;
   	...
   	else if(depth<=NIGK.TDEPTH){	//Upon restart, Winch must be at or near surface. if((Calltime-hours now)-prevTime)<3600 seconds) try calling... phase 3? What tuports needed?
   		LARA.SURFACED=true;
//   		LARA.BUOYMODE=1; //Removed after first Lake W. Deployment. This skips the ascent call in phase two 
   		if(prevTime!=0){
...
void PhaseOne(){
   ...
   //Stay here until system_timer says it's time to send data, user input for different phase, NIGK R    
...
void PhaseTwo(){
   ...
   while((!LARA.SURFACED||LARA.BUOYMODE==1)&&LARA.PHASE==2){
      ...
      if(LARA.DEPTH<=NIGK.TDEPTH){
      	...
      	if(LARA.TOPDEPTH<=NIGK.TDEPTH)
   		...
   		if(LARA.AVGVEL==0.0) LARA.AVGVEL=((float)NIGK.RRATE/60.0);
   ...
   flogf("\n\t|Rate of Ascent: %5.2fMeters/Minute", (depthChange/((float)timeChange/60.0)));
...
/****************************************************************************\
** Phase Three
** Testing iridium/gps connection. If failed, release winch cable another meter or two.
...
void PhaseThree(){
   ...
   OpenTUPort_WISPR(false);
	...
	while(result<=0){
			result = IRIDGPS(GlobalRestart); //-1=false gps, -2=false irid, 1=success 2= fake cmds 3 = real cmds
		...
		if(result>=1||attempts>2){
		   ...
		   if(result==1||result==2){//Upload Success, Fake Commands
		   	...
		   	if(IridiumCalls>3){
		   		ParseStartupParams(true); 	//Load default parameters in "default.cfg" file
					if(NIGK.RECOVERY){
						if(MPC.DATAXINT!=30){
							MPC.DATAXINT=30;
							VEEStoreShort(DATAXINTERVAL_NAME, MPC.DATAXINT);
				...
				if(GlobalRestart) ParseStartupParams(true);//Added 9.28.2016 after first deployment Lake W.
		   	}
			...
			else if(GlobalRestart) ParseStartupParams(true); //Added 9.28.2016 after first deployment Lake W.
		...
		//Bad GPS- GPS fails usually from bad reception.
			...
			while(LARA.DEPTH>LARA.TDEPTH){
				Incoming_Data();
				RTCDelayMicroSeconds(100000L); count++;
...
void PhaseFour(){
   ...
   OpenTUPort_NIGK(true);
	...
	PrintSystemStatus();
	//First stop buoy ( in case of restart we don't know the state of the buoy. better to stop 
	if(LARA.BUOYMODE!=0){
		Winch_Stop();
		WaitForWinch(0);
		}
	...
	/****
	ADD IN CHECK HERE. if WaitForWinch returns false because of timeout and lack of AModem Response: then check CTD Average depth and make sure Velocity>0.25 (descending.) 
	...
	****/
	...
	while(LARA.BUOYMODE==2){
   	...
   	timecheck=time(NULL);	
   	//Check depth change every 60 seconds... This is an out of the while loop. Hopefully it will only come here when TUPort to the AModem stops LARA doesn't hear the Winch serial coming.
   	if(timecheck-DescentStart>interval*60){
   		flogf("\n\t|PhaseFour() Check depth change");
   		prevDepth-=LARA.DEPTH;
   		if(prevDepth<0) prevDepth=prevDepth*-1.0;
   		if(prevDepth<3){
   			flogf("\nERROR|Depth change less than 3 meters... Winch hasn't responded STOP");
   ...
   //Total Vertical depth change. total time change, calculate estimated velocity. 
   depthChange = LARA.MOORDEPTH-LARA.TOPDEPTH;
   LARA.DESCENTTIME = (short)(DescentStop-DescentStart);
   flogf("\n\t|Rate of Descent: %fMeters/Minute", (depthChange/((float)LARA.DESCENTTIME/60.0)));
   flogf("\n\t|User calculated Cable Payout: %fMeters",  ((float)LARA.DESCENTTIME/60.0)*NIGK.FRATE);
...
int Incoming_Data(){
   bool incoming=true;
   static int count=0;
   int value=0; //Need to update this if we ever need to return a legit value. -AT 20151215
   switch(LARA.PHASE){
      ...
      case 0: 
      	while(incoming){
      		...
      		if(tgetq(NIGKPort)){
               ...
               CTD_Data();
               if((!LARA.SURFACED&&LARA.PHASE>1)||LARA.BUOYMODE>0||LARA.PHASE==0)	//if not surfaced (target depth not reached.) and winch is moving (not stopped)
               	CTD_Sample();
               }
            else if(cgetq()){
      ...
      case 2: case 4:
         ...
         while(incoming){
            ...
            if(tgetq(PAMPort)){
               ...
               //DBG(flogf("CTD Incoming");)
               CTD_Data();
               if((!LARA.SURFACED&&(LARA.PHASE==2||LARA.PHASE==4))||LARA.BUOYMODE>0)	//if not surfaced (target depth not reached.) and winch is moving (not stopped)
               	CTD_Sample();
               }
            else if(cgetq()){
               //DBG(flogf("Console Incoming");)
      ...
      default:
      	//Lost winch phase. Get LARA System Status decided which phase it best belongs.
...
void CTDSleep(void)
{
   ...
   PinBus(IRQ4RXD);                    // Console Interrupt
	...
	while (PinRead(IRQ2)&&PinRead(IRQ5)&&PinRead(IRQ3RXX)&&PinRead(IRQ4RXD)&&!data)  
...
void SleepUntilWoken(void){
   ...
   flogf("\n%s|SleepUntilWoken()", Time(NULL));
	...
	flogf("\n%s|Aquisition ended!", Time(NULL));putflush();// tell 'em we're back
//			BIOSResetToPicoDOS();
	 //}
...
DBG(else
  	...
  	//*** LARA Write ***//
   sprintf(WriteBuffer, "LARA Program Ver:%.1f\naa:bb.cccc North ddd:ee.ffff West\nFileNumber: %ld\nStarts:%d of %d\nWriteTime:%s\nSeconds:%lu\nDetection Interval:%3d\nData Inteval: %d\n\0",
      PROG_VERSION, MPC.FILENUM, MPC.STARTUPS, MPC.STARTMAX, Time(NULL), TotalSeconds, MPC.DETINT, MPC.DATAXINT);
...
char* PrintSystemStatus(){
	...
	sprintf(returnString, "LARA: %d%d%d%d\nMOORDEPTH:%5.2f\nCURRENTDEPTH:%5.2f\nTOPDEPTH:%5.2f\nTARGETDPETH:%d\nAVG.VEL:%5.2f\nCTDSAMPLES:%d\n\0" \
	,LARA.DATA ? 1 : 0,LARA.SURFACED ? 1 : 0,LARA.PHASE,   LARA.BUOYMODE, \
	LARA.MOORDEPTH, LARA.DEPTH, LARA.TOPDEPTH, LARA.TDEPTH, LARA.AVGVEL, LARA.CTDSAMPLES);
	flogf("\n%s", returnString);
...
void WaitForWinch(short expectedBuoyMode){
	...
	flogf("\n%s|WaitForWinch(): ExpectedBuoyMode: %d", Time(NULL), expectedBuoyMode);
	timenow=time(NULL);
	while(timenow<BreakTime){
		...
		if(tgetq(NIGKPort)){ 
			...
			if(expectedBuoyMode==LARA.BUOYMODE){ 
				...
				break;
			else if(expectedBuoyMode==3) return;
			}
		else if(tgetq(CTDPort)){
			CTD_Data();
...
bool CheckTime(ulong prevTime, short mode, short hour){
	...
	if(mode==0){
		...
		IRID.CALLHOUR=hours;
		if(IRID.CALLHOUR>23) IRID.CALLHOUR=23;
		else if(IRID.CALLHOUR<0) IRID.CALLHOUR=0;
